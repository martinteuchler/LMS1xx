...
int main(int argc, char **argv)
{
  ...
  int port;
  std::string echoes;
  LMS5xx::EchoFilter echo_mode = LMS5xx::EchoFilter::AllEchos;

  ros::init(argc, argv, "lms5xx");
  ros::NodeHandle nh;
  ros::NodeHandle n("~");
  ros::Publisher scan_pub = nh.advertise<sensor_msgs::LaserScan>("scan", 1);
  ros::Publisher multi_pub = nh.advertise<sensor_msgs::MultiEchoLaserScan>("multiscan", 1);

  n.param<std::string>("host", host, "192.168.0.1");
  n.param<std::string>("frame_id", frame_id, "laser");
  n.param<int>("port", port, 2111);
  n.param<std::string>("echos", echoes, "all");
  ...
  while (ros::ok())
  {
    ...
    while (ros::ok())
    {
      ...
      {
        if (echo_mode == LMS5xx::EchoFilter::AllEchos)
        {
